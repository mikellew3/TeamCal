...
// POST { token, entry: { member_id?, event_type, title?, start_date, end_date, notes?, status? } }
// Admin direct-create. Bypasses conflict rules entirely. Defaults status to 'approved'.
export default async function handler(req, res) {
  ...
  const e = body?.entry || {};
  if (!ALL_TYPES.includes(e.event_type)) {
  ...
  if (!memberId && !title) {
    return send(res, 400, { error: 'title_or_member_required' });
  }
  const status = ['pending', 'approved', 'denied'].includes(e.status) ? e.status : 'approved';
